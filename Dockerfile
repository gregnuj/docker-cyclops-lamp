...
COPY --from=gregnuj/cyclops-mariadb:alpine3.8 /usr/local/bin/sync_master.sh /usr/local/bin/sync_master.sh
COPY --from=gregnuj/cyclops-mariadb:alpine3.8 /usr/local/bin/check_replication.sh /usr/local/bin/check_replication.sh
